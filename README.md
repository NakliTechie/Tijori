...
| Attachments | v1.4. `tijori-attachments/<id>.bin` = nonce ‖ AES-256-GCM, key `HKDF(master_secret, "tijori-v2", "attachment-key:<id>")`, AAD = id. Filename / MIME / size / ciphertext SHA-256 travel only in the encrypted `attachment_added` event; `attachment_removed` is a tombstone that is never re-added. Blob is written before its event. A record whose blob is absent is a normal *pending* state (QR never carries blobs; LAN, folder sync and `.tijori` archives do). GC is user-run and deletes only tombstoned / deleted-entry blobs — never an unreferenced one, because a cloud folder can deliver a blob before the `.jsonl` that names it. Soft cap 10 MB, hard cap 50 MB. v2 vaults only. |
